#+PROPERTY: header-args :results output stderr :noweb yes
...
implements it is compatible with Harmony.  Since the comment is from 2010, it
might be that things have changed since.  I have no EcmaScript 2015-compliant
implementation to test.

In V8, a workaround is to use a function expression instead of a declaration.
But this does not seem to work for overriding functions.
...
#+BEGIN_SRC nodejs
  var m = (function(){
    ...
    with (scope) {
      ...
      function h() { with(scope){return g()} }
      return {f, g, h, scope}
    }
  }())

  console.log(m.f(), m.g(), m.h())
  m.scope.a = 2
  console.log(m.f(), m.g(), m.h())
  m.scope.g = function() { return 42 }
  console.log(m.f(), m.g(), m.h())
  m.scope.f = function() { return 42 }
#+END_SRC
...
See [[large-heap-graph.svg][large heap graph SVG]].  Best viewer seems to be Chrome, but even that is
pretty slow.
...
* Overriding objects
Overriding functions and values works great with ~with~.  However, when
overriding objects, I often find myself delegating to the existing object:
...
#+BEGIN_SRC js
  var m = (function(){
    ...
    with (scope) {
      var obj = {a: 2, b:4}
      function f() { return obj.a + obj.b }
      Object.setPrototypeOf(scope, {obj})
      return {f, scope}
    }
  }())
  ...
  m.scope.obj = {b:12}
  print(m.f())
  m.scope.obj = Object.create(m.scope.__proto__.obj)
  m.scope.obj.b = 12
  print(m.f())
#+END_SRC
...
In the value case, one has no choice but erasing it.  In the function case, we
may want only change a tiny bit of it, and we can delegate to the original
function (as in ~proceed~).  In the object case, overriding only a few values
is a legitimate scenario.  Maybe we can provide a ~replace~ function which does
the intuitive thing in all these scenarios?

#+BEGIN_SRC js
/** Create a replacement of object O by object R.
 ,*
 ,*  If O is a function, return a function that will call R with the O as first
 ,*  argument, followed by the original arguments of the call.
 ,*
 ,*  If O is an object, return a new object containing the properties of R, but
 ,*  with O as prototype.
,*/
function replacement(o, r) {
  ...
  m.scope.obj = {b:12}
  print(m.f())
  m.scope.obj = replacement(m.scope.__proto__.obj, {b:12})
  print(m.f())
#+END_SRC
...
I’ve resolved to use a simpler scheme: helper functions for exactly what you
mean.  See [[file:narcissus/lib/i13n.js][i13n.js]]



* Cleaning the code for uploading
On https://github.com/open-scope/open-scope, for double-blind reviewing
purposes.

As the reviews are done, the github account is now deleted.
